...
def train_global_gan(house_files, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        gan.train_step(real_apps)
        save_generated_signal(generator, "globaldishw")
...
def compare_generated_signals(house_ids, include_global=True):
    ...
    if include_global:
        try:
            df = pd.read_csv("generated_signal_houseglobaldishw.csv")
            global_signal = df["value"].values
            plt.figure(figsize=(10, 4))
            plt.plot(global_signal, label="Global Field", color='black')
            plt.title("Generated Signal - Global dishwasher Field")
            plt.xlabel("Time")
            plt.ylabel("Generated Value")
            plt.legend()
            plt.grid(True)
...
if __name__ == "__main__":
    house_files = {
        1: {"file": RAW_DATA_PATH / "HOUSE_1.csv", "app_column": 6},
        #2: {"file": RAW_DATA_PATH / "HOUSE_2.csv", "app_column": 5},
        3: {"file": RAW_DATA_PATH / "HOUSE_3.csv", "app_column": 4},
        4: {"file": RAW_DATA_PATH / "HOUSE_4.csv", "app_column": 3},
        5: {"file": RAW_DATA_PATH / "HOUSE_5.csv", "app_column": 6},
        8: {"file": RAW_DATA_PATH / "HOUSE_8.csv", "app_column": 6},
    }
