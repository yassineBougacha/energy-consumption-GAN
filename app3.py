...
class Discriminator(tf.keras.Model):
    def __init__(self, segment_length):
        ...
        self.model = tf.keras.Sequential([
            ...
            tf.keras.layers.LeakyReLU(0.2),
            tf.keras.layers.Conv1D(256, 25, strides=2, padding='same'),
            tf.keras.layers.LeakyReLU(0.2),
...
def train_gan(house_index, app_column, house_data, segment_length, epochs=100, batch_size=32):
    ...
    g_optimizer = tf.keras.optimizers.Adam(0.0001, beta_1=0.06) # 0.0001, beta_1=0.05(fridge)
